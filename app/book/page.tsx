...
export default function BookPage() {
  ...
  const submitBooking = async () => {
    ...
    // Use the API with Telegram alerts
    try {
      const response = await fetch('/api/bookings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          customerName: form.passenger,
          pickup: form.pickup,
      ...
      setBookingId(data.bookingId || data.booking?.id)
      ...
      try {
        ...
        // Dispatch failed but booking is saved - still show success
      }
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: '560px', paddingTop: '3rem' }}>
        ...
        <p className="text-xs text-gray-500 mt-4 text-center" style={{ fontSize: '0.75rem', color: 'var(--muted)', marginTop: '1rem' }}>
          Language: {userLanguage === 'sw' ? 'Swahili' : userLanguage === 'ki' ? 'Kikuyu' : userLanguage === 'lu' ? 'Luo' : 'English'} (auto-detected)
        </p>
      </main>
    </>
  )
